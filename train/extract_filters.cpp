...
int main (int argc, char* argv[])
{
  ...
  int K = 250; //number of filters (for visualization must be multiple of 16)

  //load training examples and extract patches

  Mat patches = Mat::zeros(0, patch_width*patch_height, CV_64FC1);
  int patch_counter = 0;
  ...
  for (int f=0; f<num_images; f++)
  {
    ...
    int num_patches_x_image = image_width/4;
  ...
  cout << "Total number of patches << " <<patch_counter << " " << patches.rows << endl;
