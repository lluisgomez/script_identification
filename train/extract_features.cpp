...
int main (int argc, char* argv[])
{
  ...
  {
    Mat src  = imread(path+filenames[f]);
    if(src.channels() != 3)
      continue;
    ...
    // Do slidding window from x=0 to src_width-image_size in three rows (top,middle,bottom)
    ...
    { 
      ...
      { 
        ...
        Mat img;
        src(Rect(x,y,image_size,image_size)).copyTo(img); // img must be a window of 32x32 pixels
